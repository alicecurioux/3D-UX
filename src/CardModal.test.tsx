...
describe("CardModal / rendering", () => {
  it("renders nothing when card is null", () => {
    const { container } = render(<CardModal card={null} width={400} onClose={vi.fn()} />);
    expect(container.firstChild).toBeNull();
  });

  it("renders number, title, description when card provided", () => {
    render(<CardModal card={TEAL_CARD} width={400} onClose={vi.fn()} />);
  ...
  it("applies teal tone class and not red", () => {
    const { container } = render(<CardModal card={TEAL_CARD} width={400} onClose={vi.fn()} />);
  ...
  it("applies red tone class and not teal", () => {
    const { container } = render(<CardModal card={RED_CARD} width={400} onClose={vi.fn()} />);
    expect(container.querySelector(".card-modal--red")).toBeInTheDocument();
    expect(container.querySelector(".card-modal--teal")).toBeNull();
  });

  it("sets width style from prop", () => {
    const { container } = render(<CardModal card={TEAL_CARD} width={320} onClose={vi.fn()} />);
    const modal = container.querySelector(".card-modal") as HTMLElement;
    expect(modal.style.width).toBe("320px");
  });

  it("sets fractional width (e.g. 337.5) from prop", () => {
    const { container } = render(<CardModal card={TEAL_CARD} width={337.5} onClose={vi.fn()} />);
    const modal = container.querySelector(".card-modal") as HTMLElement;
...
describe("CardModal / close interactions", () => {
  it("calls onClose when backdrop is clicked", async () => {
    const onClose = vi.fn();
    render(<CardModal card={TEAL_CARD} width={400} onClose={onClose} />);
...
describe("CardModal / event listener lifecycle", () => {
  it("does NOT attach any keydown listener when card is null", () => {
    ...
    render(<CardModal card={null} width={400} onClose={vi.fn()} />);
  ...
  it("removes keydown listener on unmount", () => {
    ...
    const { unmount } = render(<CardModal card={TEAL_CARD} width={400} onClose={vi.fn()} />);
    unmount();
  ...
  it("removes keydown listener when card changes from a value to null", () => {
    const spy = vi.spyOn(document, "removeEventListener");
    const { rerender } = render(<CardModal card={TEAL_CARD} width={400} onClose={vi.fn()} />);
    rerender(<CardModal card={null} width={400} onClose={vi.fn()} />);
    const keydownRemovals = spy.mock.calls.filter(([ev]) => ev === "keydown");
  ...
  it("Escape does NOT fire onClose after component unmount (no dangling listener)", () => {
    const onClose = vi.fn();
    const { unmount } = render(<CardModal card={TEAL_CARD} width={400} onClose={onClose} />);
    unmount();
    fireEvent.keyDown(document, { key: "Escape" });
    expect(onClose).not.toHaveBeenCalled();
  });
  ...
  it("Escape does NOT fire onClose after card transitions to null", () => {
    const onClose = vi.fn();
    const { rerender } = render(<CardModal card={TEAL_CARD} width={400} onClose={onClose} />);
    rerender(<CardModal card={null} width={400} onClose={onClose} />);
    fireEvent.keyDown(document, { key: "Escape" });
    expect(onClose).not.toHaveBeenCalled();
  });
});
